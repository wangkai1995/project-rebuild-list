...
//menu标签激活状态
var navLabelActiveChange = function(index,nextIndex){
    var li = document.querySelectorAll('#menu li');
    for(var i=0; i<li.length; i++){
        li[i].className = '';
    }
    li[nextIndex-1].className = 'active';
    //重置位置
    if(nextIndex === 4){
        var item = document.querySelectorAll('.server-container .server-item');
...
//页面切换
var pageChange = function(anchorLink ,index){
    if(index === 4){
        var item = document.querySelectorAll('.server-container .server-item');
        var time ;
        for(var i=0 ;i<item.length; i++){
            (function(count){
                var n  =count;
                setTimeout(function(){
                    stopMove[n] = new window.move().collision([0, 1], 2000, function(v){
                            item[n].style.top = v*260 + 'px';   
                    });
                 }, n*200);
            })(i);
        }
...
$(function(){
    ...
    $('#qnche-home').fullpage({
        ...
        loopBottom:true,
        afterLoad:pageChange,
        onLeave:navLabelActiveChange,
    });
})
